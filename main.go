...
func startTraining(charClass, char_class string) string {
	if charClass == "warrior" {
	...
	for cmd != "skip" {
		...
		switch {
		case cmd == "attack":
			fmt.Println(attack(charClass, char_class))
		case cmd == "deffence":
			fmt.Println(defence(charClass, char_class))
		case cmd == "special":
			fmt.Println(special(charClass, char_class))
		default:
